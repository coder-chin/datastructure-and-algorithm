...
int CreateHuffmanTree(HuffmanTree& HT, string str) {
	map<char, int> mymap;  //映射

	for(unsigned int i=0; i<str.length(); i++) {
		mymap.find(str[i]) != mymap.end() ? mymap[str[i]]++ : mymap[str[i]] = 1; //比较该字符串与map中最后一个是否相等
	}

	unsigned int i = 0;

	for(auto it=mymap.begin(); i<mymap.size()-1; i++) {
		cout << it->first << ":" << it->second << " ";
		it++;
	}

 	cout << (--mymap.end())->first << ":" << (--mymap.end())->second;  //最后一个
	cout<<endl;

	int n = mymap.size();
	HT = new HuffmanNode[2*n];   //初始化HT
